...
#IDH_tub病例，对应label设为'tub',输出data尺寸为4*32*240*240
#data.dtype==float32,label.dtype==int64
tub_path=data_path('/grade/GBM_4')
for idx,tub_patient in enumerate(tub_path):
    class_=generate_datalabel(tub_patient,'tub')
    #每个tub_patient为一个病人的flair、seg、t1、t1ce、t2列表（按顺序）:
    for modal in range(len(tub_patient)-1):
...
yes_path=data_path('/grade/LGG_2')
yes_dataset=[]
yes_labelset=[]
...
training_loader=unilsdata.DataLoader(Loading(
    dataset_training,labelset_training),batch_size=2,shuffle=True,drop_last=True,**kwargs)
testing_loader=unilsdata.DataLoader(Loading(
    dataset_testing,labelset_testing),batch_size=2,shuffle=True,drop_last=True,**kwargs)
...
def train(epoch):
    ...
    print('--------------------------------------------') 
    print('HGG&&LGG_Fold5:Train Epoch:{}-->Loss:{:6f}-->ACC:{:6f}-->SN:{:6f}-->SP:{:6f}-->PPV:{:6f}-->NPV:{:6f}-->AUC:{:6f}'
...
def test(epoch):
    ...
    print('--------------------------------------------') 
    print('HGG&&LGG_Fold5:Test Epoch:{}-->Loss:{:6f}-->ACC:{:6f}-->SN:{:6f}-->SP:{:6f}-->PPV:{:6f}-->NPV:{:6f}-->AUC:{:6f}'
